...
- **Base de datos**: PostgreSQL (`marcas_db`)
...
Este comando:
1. Arranca PostgreSQL si está parado
2. Crea la base de datos `marcas_db` si no existe
3. Aplica `backend/schema.sql`
4. Genera `backend/.env` automáticamente
5. Lanza el servidor con `nodemon`
...
Definidas en `backend/.env` (generado por `scripts/setup-db.sh`).
